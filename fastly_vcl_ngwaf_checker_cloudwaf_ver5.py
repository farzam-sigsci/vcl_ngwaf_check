...
# Fastly API base URL for efficient and reliable API interactions
...
def check_cloudwaf(api_token, service_id, version):
    """Inspect the entire compiled VCL for a host definition containing sigscicloudwaf.com."""
    #pattern = r'^\s*\.host\s*=\s*"[^"]*\.sigscicloudwaf\.com"\s*;?\s*$'
    pattern = r'\.host\s+=\s+".*sigscicloudwaf\.com"\s*(?:;)?'
    
    # Check compiled VCL for CloudWAF host
    ...
    if generated_vcl_status == 200 and isinstance(generated_vcl_data, dict):
        ...
        if re.search(pattern, content, re.IGNORECASE):
            print(f"  CloudWAF host found in Service ID: {service_id}, Version: {version}, Generated VCL (Regex match)")
            print(f"    Matched content: {content.strip()[:100]}...")
            #return True
            return "✅"
        print(f"  CloudWAF host NOT found in Service ID: {service_id}, Version: {version} (Generated VCL)")
    ...
    print(f"  CloudWAF host NOT found in Service ID: {service_id}, Version: {version}")
    #return False
    return "❌"
...
def main():
    """Main function to audit NGWAF snippets and client challenge settings across all services in a single pass."""
    ...
    report_file = f"account_report_{customer_id}_{timestamp}.csv"
    ...
    for service in services:
        ...
        cloudwaf_status = "No active version"
        ...
        if active_version and active_version != "None" and active_version != "Unknown":
            ...
            cloudwaf_status = check_cloudwaf(api_token, service_id, active_version)
            client_challenge_enabled = check_client_challenge(api_token, service_id, active_version)

        print(f"  NGWAF Status: {waf_status}")
        print(f"  CloudWAF Status: {cloudwaf_status}")
        print(f"  Client Challenge Enabled: {client_challenge_enabled}")
        service_results.append([service_name, service_id, active_version, waf_status, cloudwaf_status, str(client_challenge_enabled)])
    ...
    for filename in [report_file]:
        with open(filename, "w", newline="") as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(["Service Name", "Service ID", "Active Version", "NGWAF Status", "CloudWAF Status", "Client Challenge Enabled"])
